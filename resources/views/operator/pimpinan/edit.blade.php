...
<div class="max-w-4xl mx-auto">
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-200 bg-slate-50 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">Edit Profil Pimpinan</h2>
                <p class="text-sm text-slate-500 mt-0.5">Memperbarui data profil atas nama <span class="font-bold text-slate-700">{{ $pimpinan->nama_lengkap }}</span>.</p>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('operator.pimpinan.update', $pimpinan->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status_aktif" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                            ...
                            <option value="0" {{ old('status_aktif', $pimpinan->status_aktif) == '0' ? 'selected' : '' }}>Nonaktif / Pensiun / Pindah</option>
                        </select>
                    </div>
                    ...
                    <div></div> <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div class="md:col-span-1">
                            ...
                            <input type="text" name="gelar_depan" value="{{ old('gelar_depan', $pimpinan->gelar_depan) }}" placeholder="Drs." class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Nama Lengkap *</label>
                            <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $pimpinan->nama_lengkap) }}" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                        </div>
                        <div class="md:col-span-1">
                            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Gelar Belakang</label>
                            <input type="text" name="gelar_belakang" value="{{ old('gelar_belakang', $pimpinan->gelar_belakang) }}" placeholder="M.Pd." class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status_pegawai" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                            ...
                            @php $statusList = ['PNS', 'PPPK', 'Honorer', 'GTY', 'GTT']; @endphp
                            @foreach($statusList as $status)
                                <option value="{{ $status }}" {{ old('status_pegawai', $pimpinan->status_pegawai) == $status ? 'selected' : '' }}>{{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" value="{{ old('nip', $pimpinan->nip) }}" placeholder="18 digit angka NIP" class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Pangkat / Golongan</label>
                        <input type="text" name="pangkat_gol" value="{{ old('pangkat_gol', $pimpinan->pangkat_gol) }}" placeholder="Contoh: Pembina Tk. I / IV/b" class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Jabatan Struktural</label>
                        <input type="text" name="jabatan" value="{{ old('jabatan', $pimpinan->jabatan) }}" placeholder="Contoh: Kepala Sekolah" class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="jenis_kelamin" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                            <option value="L" {{ old('jenis_kelamin', $pimpinan->jenis_kelamin) == 'L' ? 'selected' : '' }}>Laki-laki</option>
                            <option value="P" {{ old('jenis_kelamin', $pimpinan->jenis_kelamin) == 'P' ? 'selected' : '' }}>Perempuan</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="agama" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                            @foreach(['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu'] as $agm)
                                <option value="{{ $agm }}" {{ old('agama', $pimpinan->agama) == $agm ? 'selected' : '' }}>{{ $agm }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $pimpinan->tempat_lahir) }}" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Tanggal Lahir *</label>
                        <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $pimpinan->tanggal_lahir) }}" required class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Nomor HP / WA</label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $pimpinan->no_hp) }}" class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Alamat Lengkap</label>
                        <textarea name="alamat" rows="3" class="w-full rounded-lg border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-colors">{{ old('alamat', $pimpinan->alamat) }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($pimpinan->foto)
                            <div class="mb-3 flex items-center gap-3">
                                <img src="{{ asset('storage/' . $pimpinan->foto) }}" alt="Foto Lama" class="h-12 w-12 rounded-lg object-cover border border-slate-200">
