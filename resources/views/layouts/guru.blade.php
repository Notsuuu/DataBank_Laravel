        ...
        <div class="border-t border-slate-100 pt-4 mx-2">
            <div class="flex items-center justify-between rounded-xl bg-slate-50 p-3 border border-slate-100">
                <div class="flex items-center gap-2.5 overflow-hidden">
                    <div
                        class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-emerald-100 font-bold text-emerald-700 text-sm ring-2 ring-white">
                        {{ Str::upper(substr(auth()->user()->name ?? 'GR', 0, 2)) }}
                    </div>
                    <div class="overflow-hidden">
                        <span
                            class="block truncate text-xs font-bold text-slate-800">{{ auth()->user()->name ?? 'Nama Guru' }}</span>
                        <span class="block text-[10px] font-semibold text-slate-400 truncate">Tenaga Pendidik</span>
                    </div>
                </div>
