...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 border-b border-slate-200/80 pb-5">
        ...
        <div>
            ...
            <p class="text-sm font-semibold text-slate-500 mt-0.5">Unggah salinan dokumen fisik asli Anda untuk validasi kepegawaian.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-4 bg-slate-50/50 rounded-2xl border border-slate-200/80 p-5 shadow-sm">
            ...
            <form action="{{ route('guru.berkas.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="jenis_berkas" required class="w-full rounded-xl border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-colors shadow-sm">
                ...
                <div>
                    ...
                    <input type="file" name="file_berkas" required accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-xs text-slate-500 file:mr-3 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer outline-none bg-white border border-slate-300 rounded-xl shadow-sm transition-colors">
                    <p class="text-[10px] font-semibold text-slate-400 mt-1.5">Format: JPG, PNG, PDF. Maksimal 5MB.</p>
                </div>

                <div class="pt-2">
                    <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl text-sm transition-all shadow-sm shadow-emerald-500/30 flex justify-center items-center gap-2">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                @if($guru->file_ktp)
                                    <a href="{{ asset('storage/' . $guru->file_ktp) }}" target="_blank" class="text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors">Lihat Dokumen</a>
                                @else
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                @if($guru->file_ijazah)
                                    <a href="{{ asset('storage/' . $guru->file_ijazah) }}" target="_blank" class="text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors">Lihat Dokumen</a>
                                @else
                                    <span class="text-sm font-semibold text-slate-400">-</span>
                                @endif
                            </td>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                @if($guru->file_sk)
                                    <a href="{{ asset('storage/' . $guru->file_sk) }}" target="_blank" class="text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors">Lihat Dokumen</a>
                                @else
                                    <span class="text-sm font-semibold text-slate-400">-</span>
                                @endif
                            </td>
